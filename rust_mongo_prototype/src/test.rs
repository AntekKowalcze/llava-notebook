...
async fn test() {
    ...
    let v = crate::db::connecting_to_db().await.unwrap();
    let vec: Vec<Note> = vec![note1, note2.clone(), note3.clone(), note4, note5];
    for note in &vec {
        crate::db::inserting_note(&v, note).await.unwrap();
    }
    let vec: Arc<RwLock<Vec<crate::models::Note>>> = Arc::new(RwLock::new(vec));
    let test_vec = crate::helpers::find_note_by_title_with_connection(&v, vec)
        .await
        .unwrap();
    let test_vec2 = vec![note2.clone(), note3.clone(), note2, note3];
