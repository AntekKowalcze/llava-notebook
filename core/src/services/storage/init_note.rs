...
//  after function which saves note content manually there should be command which increents activity number in sql database
//  TREAT CRETING NOTE AS EDITING SO RUN ADDING ACTIVITY COUNT AFTER CREATION AND DELETATION look at queries in dashboard to create good insert queries in adding note
fn init_note(
