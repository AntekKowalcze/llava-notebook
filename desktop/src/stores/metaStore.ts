...
export const useMetaStore = defineStore('metaStore', () => {
      ...
      if (userConfig.config['local.mode'] === 'off') {
        invoke<[boolean, boolean]>('get_connection_status')
          .then(([server, internet]) => {
            if (isConnectedToServer.value === null) {
              isConnectedToServer.value = server;
            }

            if (isConnectedToInternet.value === null) {
              isConnectedToInternet.value = internet;
            }
          })
          .catch(() => {});
      }
  ...
  const connectionStatus = computed<'unknown' | 'offline' | 'server-unreachable' | 'online'>(() => {
    if (isConnectedToInternet.value === null || isConnectedToServer.value === null)
      return 'unknown';
