...
<template>
    <div class="flex flex-col flex-1">
        <ArrowBigLeftDash
            class="text-note-paprika/80 absolute top-[93%] left-[2%] transition-transform duration-200 hover:scale-95 click:scale-90"
            @click="redirect">
        </ArrowBigLeftDash>
        <!-- greeting -->
        <div class="relative flex items-center justify-center py-10 mt-4">
            <div class="absolute left-[12.5%]">
                <IconComponent :width="'w-44'" :height="'h-44'" />
            </div>
            <div class="flex flex-col items-center gap-3">
                <h1 class="text-note-ivory text-5xl font-semibold tracking-tight">
                    {{ greeting }} <span class="text-note-paprika">{{ username }}</span>
                </h1>
                <p class="text-note-pumice/25 text-xs tracking-widest mb-4">
                    #{{ userId }}
                </p>
                <p class="text-note-ivory/80 text-sm tracking-widest uppercase">
        ...
        <div class="flex-1 flex-col items-start">
            <div class="h-fit mb-4">
                <p class="text-note-ivory text-3xl font-semibold ml-[12.5%] mt-10 mb-0">
                    This is your activity in Llava app
                </p>
            </div>

            <div class="h-1/5 flex flex-row w-full justify-evenly items-center mt-0">
                <!-- number of notes card -->
                <DashboardNumberCard :text="'number of notes'" :count="numberOfNotes">
                    <img :src="noteSvg" class="h-16 w-16" />
                </DashboardNumberCard>

                <DashboardNumberCard :text="'number of encrypted notes'" :count="numberOfEncrypted">
                    <LockIcon class="text-note-ivory w-16 h-16"></LockIcon>
                </DashboardNumberCard>
                <DashboardNumberCard :text="'account age in days'" :count="accountAgeDays">
                    <Calendar class="text-note-ivory w-16 h-16">
                    </Calendar>
                </DashboardNumberCard>
            </div>
            <!-- grid wrapper -->
            ...
            <div class="flex flex-1">
                <div class="w-full flex flex-col gap-4 px-[12.5%] py-4">
                    <div class="flex items-baseline justify-between">
                        <p class="text-note-ivory text-xl font-semibold">
                            Activity heatmap
                        </p>
                        <p class="text-xs tracking-widest uppercase text-note-pumice/60">
                            Last 365 days
                        </p>
                    </div>
                    <!-- month names  -->
                    <div class="flex flex-row justify-evenly h-4 w-[92%] text-note-pumice/60 select-none"><span
                            v-for="(label, idx) in monthLabels" :key="idx"
                            class="h-4 text-[10px] leading-4 text-note-pumice/60">{{ label }}</span></div>
                    <div class="overflow-x-auto pb-2">
                        <div class="flex gap-3">
                            <!-- day names -->
                            <div class="flex flex-col justify-between py-[2px]">
                                <span v-for="(label, idx) in weekdayLabels" :key="idx"
                                    class="h-4 text-[10px] leading-4 text-note-pumice/60 select-none">
                                    {{ label }}
                                </span>
                            </div>
                            ...
                            <div class="inline-grid grid-flow-col grid-rows-7 gap-1">
                                <ActivityMapSquare v-for="day in weeks.flat()" :key="day.date" :date="day.date"
                    ...
                    <div class="flex items-center gap-2 text-[11px] text-note-pumice/70">
                        ...
                        <div class="flex gap-1">
                            <div class="w-3 h-3 rounded-[3px] bg-note-graphite border border-note-pumice/40" />
                            <div class="w-3 h-3 rounded-[3px] bg-note-paprika/40" />
                            <div class="w-3 h-3 rounded-[3px] bg-note-paprika/70" />
                            <div class="w-3 h-3 rounded-[3px] bg-note-paprika" />
                        </div>
                        <span>More</span>
                    </div>
                </div>
    ...
    <div class="mt-6 px-[12.5%] pb-10">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Recently edited -->
            <div class="rounded-2xl border border-note-pumice/20 bg-note-graphite/60 px-5 py-4 flex flex-col gap-3">
                <div class="flex items-center justify-between">
                    <p class="text-note-ivory text-lg font-semibold">
                        Recently edited
                    </p>
                ...
                <div v-if="lastThreeEdited.length" class="flex flex-col gap-2">
                    <div v-for="([noteId, ts], idx) in lastThreeEdited" :key="noteId + ts"
                        class="flex items-center justify-between rounded-xl bg-note-graphite px-3 py-2 border border-note-pumice/15">
                        <div class="flex flex-col">
                            <span class="text-note-ivory text-sm font-medium">
                                Note #{{ idx + 1 }}
                            </span>
                            <span class="text-note-pumice/70 text-xs">
                                {{ noteId }}
                            </span>
                        </div>
                        <span class="text-note-pumice/60 text-xs">
                            {{ new Date(ts).toLocaleString() }}
                        </span>
                    </div>
                </div>

                <p v-else class="text-note-pumice/50 text-sm">
            ...
            <div class="rounded-2xl border border-note-pumice/20 bg-note-graphite/60 px-5 py-4 flex flex-col gap-3">
                ...
                <div v-if="favouriteTags.length" class="flex flex-col gap-2">
                    <div v-for="([tagName, color], idx) in favouriteTags" :key="tagName"
                        class="flex items-center justify-between rounded-xl bg-note-graphite px-3 py-2 border border-note-pumice/15">
