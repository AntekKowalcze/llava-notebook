...
async function changePassword() {
    try {
        await invoke<void>('change_password', {
            username, password: password.value, passwordRepeated: repeatPassword.value, code: authStore.pendingCode
        })
...
<template>
    <FormCard header-text="Change password" sub-text="">
        ...
        <FormButtons :disabled="!canSubmit" :content="'Submit'" @click="changePassword"></FormButtons>
        <RouterLink to="/loading" class="mt-8 mb-0 text-note-ivory/80 hover:underline">Go to main
        </RouterLink>
    </FormCard>
</template>